...
12. ✅ 操作层事务回滚 (Snapshot & Rollback)
13. ✅ 交互式 AI Provider 配置界面 (支持 API 和 CLI Coding Agents)
14. ✅ AI 操作计划生成入口
15. ✅ 多触发器上下文压缩
16. ✅ 交互式生成面板
17. ✅ AI 代码生成与触发器解释入口
18. ✅ 多触发器关联分析
19. 待完成: 构建完整触发器 ↔ 自然语言转换器
...
- `YDAgentServerWorker.lua` 中的全局变量脚本解析已经从“只读 `globals` 声明”扩展为“同时读 `InitGlobals` 赋值”，因此 RPC 层现在可以正确返回类似 `udg_compose_stage="armed"` 这类初始化后值。
- `agent.set_global_value` 的底层 C++ 内存写入仍未恢复；设计上继续保持 no-op，避免破坏 WE 内部结构。
- 当前已验证可作为安全交付路径的是：
  - 地图级文件编辑后重新编译回读
  - 真实会话中读回最终全局值
  - 物编与触发器修改通过离线构造地图 + 真实 YDWE 编译验证
- 当前未验证为稳定产品能力的是：
  - 直接在打开的真实会话里，通过持久 `*.w3xTemp` 工作区对全局变量做在线 CRUD；该 temp 目录不是稳定 LNI 工作树，不应作为正式设计前提。

---

*设计文档更新: 2026-05-08 (补充全局值准确回读、地图级全局 CRUD 交付路径、在线 temp 工作区限制说明)*
