...
struct same_size {
    static constexpr bool value = (!std::is_reference_v<InputClass> && sizeof(OutputClass) == sizeof(InputClass)) ||
                                  (std::is_reference_v<InputClass> &&
                                   sizeof(OutputClass) == sizeof(std::add_pointer_t<InputClass>));
};
...
inline R this_call(F f, This t, Args... args) {
    return (reinterpret_cast<R(__fastcall*)(typename call_::cast_type<This>::type, void*,
                                            typename call_::cast_type<Args>::type... args)>(f))(call_::cast(t), 0,
                                                                                                call_::cast(args)...);
}
} // namespace base
